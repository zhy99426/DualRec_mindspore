...
class DualRecDataloader(AbstractDataloader):
    ...
    def get_train_loader(self):
        ...
        dataloader = ds.GeneratorDataset(dataset, ["input_ids"], shuffle=True)
        dataloader = dataloader.batch(batch_size=self.train_batch_size, num_parallel_workers=self.num_workers)
        return dataloader
    ...
    def _get_eval_loader(self, mode):
        ...
        dataloader = ds.GeneratorDataset(dataset, ["input_ids","labels","candidates"], shuffle=False)
        dataloader = dataloader.batch(batch_size=batch_size, num_parallel_workers=self.num_workers)
...
class DualRecTrainDataset():
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.num_seg):
            ...
            ret["input_ids"].insert(0, np.array(seg))
    ...
    def neg_sample(self, item_set, size):
        negs = []
        while len(negs) < size:
            item = np.random.randint(1, self.num_items, (1,))[0]
...
class DualRecEvalDataset():
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.num_seg):
            ...
            ret["input_ids"].append(np.array(seg))
        # stack different segment together
        ret = {k: np.stack(v) for k, v in ret.items()}
        return (ret["input_ids"], ret["labels"], ret["candidates"])
